...
def get_all_list(vendor,account,region,group):
    ...
    url = f'{consul_url}/agent/services?filter=Service == selfflink_exporter {vendor} {account} {region} {group}'
...
def get_service():
    ...
    response = requests.get(f'{consul_url}/agent/services?filter=Service == selfflink_exporter', headers=headers)
    if response.status_code == 200:
        info = response.json()
        all_list = [i['Meta'] for i in info.values()]
        vendor_list = sorted(list(set([i['vendor'] for i in all_list])))
